...
export function AddTodoInput({ onAdd, focusSignal }: Props) {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-3 border-t border-white/[0.04] bg-white/[0.01]">
      <input
        ref={inputRef}
        className="flex-1 h-11 px-3 bg-white/[0.03] border border-white/[0.14] rounded-lg text-zinc-100 placeholder-zinc-500 outline-none text-[17px] font-medium tracking-[-0.015em] focus-visible:border-teal-200/80 focus-visible:outline-[3px] focus-visible:outline-teal-300/85 focus-visible:outline-offset-0"
      ...
      <button
        ...
        disabled={!value.trim()}
        className={cn(
          'h-11 flex items-center gap-1.5 px-3 py-1.5 text-[15px] font-semibold rounded-md active:scale-[0.96]',
